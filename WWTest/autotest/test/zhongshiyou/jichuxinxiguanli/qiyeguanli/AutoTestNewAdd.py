...
class TestNewAddClass(unittest.TestCase):  # 创建测试类
    ...
    def definenewadd(self, num, depend_click_case_id,
        ...
        if is_cancel:
            ...
            # self.activebrowser.delayTime(3000)
...
def __generateTestCases():
    ...
    newaddandchecktestcase_all = NewAddAndCheck.objects.filter(test_project="中石油_迭代一").filter(is_run_case=True).\
        filter(id=178).order_by('id')
