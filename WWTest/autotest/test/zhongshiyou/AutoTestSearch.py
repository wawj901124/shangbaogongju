...
from WWTest.autotest.config.zhongshiyou.depend.clickAndBackDepend import ClickAndBackDepend,clickandbackdepend
    #导入 ClickAndBack依赖
from WWTest.autotest.config.zhongshiyou.depend.searchInputTapInputText import SearchInputTapInputText,searchinputtapinputtext
from WWTest.autotest.config.zhongshiyou.depend.searchSelectTapSelectOption import SearchSelectTapSelectOption,searchselecttapselectoption
...
class TestSearchClass(unittest.TestCase):  # 创建测试类
    ...
    def definenewadd(self,num,depend_click_case_id,
        ...
        else: #否则断言“暂无数据”在页面中,“数据加载中”不在页面中
    ...
    for searchandchecktestcase in searchandchecktestcase_all:
        ...
        for i in range(1, forcount + 1):  # 循环，从1开始
            ...
            setattr(TestSearchClass,
                    'test_func_%s_%s_%s_%s_%s_%s' % (searchandchecktestcaseid,searchandchecktestcase.test_project,
                                                     searchandchecktestcase.test_module,searchandchecktestcase.test_page,
