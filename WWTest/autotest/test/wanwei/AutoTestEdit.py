...
# 导入 ClickAndBack依赖
from WWTest.autotest.config.wanwei.depend.editdepend.inputTapInputText import InputTapInputText, inputtapinputtext
from WWTest.autotest.config.wanwei.depend.editdepend.selectTapSelectOption import SelectTapSelectOption, selecttapselectoption
from WWTest.autotest.config.wanwei.depend.editdepend.inputTapInputFile import InputTapInputFile, inputtapinputfile
from WWTest.autotest.config.wanwei.depend.editdepend.assertTipText import AssertTipText, asserttiptext
from WWTest.autotest.config.wanwei.depend.editdepend.inputTapInputDateTime import inputtapinputdatetime, InputTapInputDateTime
from WWTest.autotest.config.wanwei.depend.editdepend.radioAndReelectionLabel import RadioAndReelectionLabel, \
    radioandreelectionlabel
from WWTest.autotest.config.wanwei.depend.editdepend.iframeBodyInputText import iframebodyinputtext
...
def __generateTestCases():
    ...
    editandchecktestcase_all = EditAndCheck.objects.filter(test_project="中石油_迭代一").\
        filter(test_module="用户管理").filter(id=11).filter(is_run_case=True).order_by('id')
    ...
    for editandchecktestcase in editandchecktestcase_all:
        ...
        for i in range(1, forcount + 1):  # 循环，从1开始
            ...
            setattr(TestEditClass,
                    'test_func_%s_%s_%s' % (
                    editandchecktestcaseid, editandchecktestcase.test_case_title, forcount_i),
