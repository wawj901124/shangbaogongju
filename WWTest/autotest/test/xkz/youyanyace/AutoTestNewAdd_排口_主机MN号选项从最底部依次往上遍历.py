...
class TestNewAddClass(unittest.TestCase):  # 创建测试类
    ...
    def definenewadd(self,num,depend_click_case_id,
        ...
        #获取ul中所有的li元素
        son_ele_s = self.activebrowser.getFatherSonElesList("xpath", ul_xpath, "tag_name", "li")
        #获取当前选项的总个数
        son_count = len(son_ele_s)
        #要点击的项数
        option_count = str(son_count-int(MN_devices_number)+1)
        li_xpath = "%s/li[%s]/span" % (ul_xpath, option_count)
        #点击选项
        self.activebrowser.findEleAndClick(num, "xpath", li_xpath)
        #主机MN号重新选择完成
