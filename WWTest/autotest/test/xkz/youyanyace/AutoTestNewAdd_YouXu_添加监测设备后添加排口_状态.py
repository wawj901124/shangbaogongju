...
from WWTest.autotest.config.wanwei.depend.newAddAndCheckDepend_PaiKou import newaddandcheckdepend
...
class TestNewAddClass(unittest.TestCase):  # 创建测试类
    ...
    def definenewadd(self,num,depend_click_case_id,
        ...
        #MN重新填写
        MN_xpath = "/html/body/div[1]/div/div/div[2]/div/div[1]/div[2]/div[2]/div[2]/div/div[2]/div/form/div[7]/div/div[1]/input"
        ...
        if is_cancel:
            ...
            print("新增加的设备：%s" % MN_devices_number)
            #继续添加排口
            if depend_add_case_id != None:
                print("depend_add_case_id：%s" % depend_add_case_id)
                devicesName_list = newaddandcheckdepend.newaddandcheckdepend(self.activebrowser, depend_add_case_id,MN_devices_number)
                self.activebrowser.outPutMyLog("依赖添加场景函数执行完毕！！！")
...
def __generateTestCases():
    ...
    newaddandchecktestcase_all = NewAddAndCheck.objects.\
        filter(test_project="油烟压测").filter(id=161).order_by('id')
    ...
    for newaddandchecktestcase in newaddandchecktestcase_all:
        ...
        for i in range(1, forcount + 1):  # 循环，从1开始
            ...
            jishu = int("%s0000"%starttime)
            MN_devices_number = str(jishu+i)
            ...
            args.append(newaddandchecktestcase.depend_new_add_and_check_case_id)
