...
for i in range(0, shishiquxian_biaotou_list_len):
    if "AVG" in shishiquxian_biaotou_list[i]:
        if "XZ_AVG" not in shishiquxian_biaotou_list[i]:
            ...
            if len(tbfactor)>0:
                ...
                shishiquxian_biaotou_list[i] = "%s(%s(%s))"%(shishiquxian_biaotou_list[i],biaotou_list_yinzi_name,biaotou_list_yinzi_danwei)
                print("-----------------")
        else:
...
mysql_yuju = """
        SELECT * FROM td_audit_2061_%s  ORDER by DT DESC limit 131
        """% (tbStationid)
# cursor_dcloud_base.execute(mysql_yuju)
# # 数据库提交命令
# # self.conn.commit()  # 执行update操作时需要写这个，否则就会更新不成功
