...
mysql_biaotou = """
  SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'dcloud_base' AND TABLE_NAME = 'tb_run_rate_year_factor'
"""
# cursor_dcloud_base.execute(mysql_yuju)
# # 数据库提交命令
# # self.conn.commit()  # 执行update操作时需要写这个，否则就会更新不成功
...
mysql_yuju = """
        SELECT * FROM  tb_run_rate_year_factor WHERE STATION_ID='%s' ORDER by DT DESC limit 131
        """% (tbStationid)
# cursor_dcloud_base.execute(mysql_yuju)
# # 数据库提交命令
# # self.conn.commit()  # 执行update操作时需要写这个，否则就会更新不成功
