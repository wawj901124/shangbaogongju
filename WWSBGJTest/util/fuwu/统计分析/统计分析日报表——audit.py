...
mysql_biaotou = """
  SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'dcloud_data' AND TABLE_NAME = 'td_audit_2061_%s'
""" % tbStationid
# cursor_dcloud_base.execute(mysql_yuju)
# # 数据库提交命令
# # self.conn.commit()  # 执行update操作时需要写这个，否则就会更新不成功
...
for i in range(0, shishiquxian_biaotou_list_len):
    if "AVG" in shishiquxian_biaotou_list[i]:
        ...
        if len(tbfactor)>0:
            ...
            print("-----------------")
    elif "XZ_AVG" in shishiquxian_biaotou_list[i]:
        biaotou_list_yinzi_code = shishiquxian_biaotou_list[i].split("_")[2]
        print("***************************************************************")
        print(biaotou_list_yinzi_code)
        print("***************************************************************")
        # 根据因子code查询tb_factor得到因子名称
        ...
        if len(tbfactor)>0:
            ...
            biaotou_list_yinzi_danwei = tbfactor[0][4]
            print(biaotou_list_yinzi_name)
            shishiquxian_biaotou_list[i] = "%s(%s(%s(%s)))"%(shishiquxian_biaotou_list[i],biaotou_list_yinzi_name,biaotou_list_yinzi_danwei,"修正平均值")
            print("-----------------")
...
mysql_yuju = """
        SELECT * FROM td_audit_2061_%s  WHERE DT like '%%2020-01-17 %%' ORDER by DT DESC limit 131
        """% (tbStationid)
# cursor_dcloud_base.execute(mysql_yuju)
# # 数据库提交命令
# # self.conn.commit()  # 执行update操作时需要写这个，否则就会更新不成功
...
shishiquxian = connectDcloudDataMyDBAndSelect(mysql_yuju)
...
for shishiquxian_one in shishiquxian:
    ...
    print("99999999999999999999999999999999999999999")
    print(len(shishiquxian_one_list))
    print("8888888888888888888888888888888888888888888888888888888888888888888888888888888")
    print(shishiquxian_one_list[104])
    print("8888888888888888888888888888888888888888888888888888888888888888888888888888888")
    if shishiquxian_one_list[104]== None:
        avg_ph_list.append(shishiquxian_one_list[78])
    else:
        avg_ph_list.append(shishiquxian_one_list[104])

    cou_ph_list.append(shishiquxian_one_list[132])
...
print(cou_ph_list )
choose(cou_ph_list)
s = [148.905, 149.017, 152.711, 150.851, 151.474, 151.113, 150.318, 150.355, 151.926, 150.362]
choose(s)
print("--------------------------------------------")
