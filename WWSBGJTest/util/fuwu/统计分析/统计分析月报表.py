...
mysql_biaotou = """
  SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'dcloud_data' AND TABLE_NAME = 'td_raw_2031_%s'
""" % tbStationid
# cursor_dcloud_base.execute(mysql_yuju)
# # 数据库提交命令
# # self.conn.commit()  # 执行update操作时需要写这个，否则就会更新不成功
...
mysql_yuju = """
        SELECT * FROM td_raw_2031_%s  ORDER by DT DESC limit 131
        """% (tbStationid)
# cursor_dcloud_base.execute(mysql_yuju)
# # 数据库提交命令
# # self.conn.commit()  # 执行update操作时需要写这个，否则就会更新不成功
...
for shishiquxian_one in shishiquxian:
    ...
    if shishiquxian_one_list[25] == None:
        avg_ph_list.append(0)
    else:
        avg_ph_list.append(shishiquxian_one_list[25])
    print("99999999999999999999999999999999999999999")
    if shishiquxian_one_list[34] ==  None:
        cou_ph_list.append(0)
    else:
        cou_ph_list.append(shishiquxian_one_list[34])
...
print("--------------------------------------------")
s = [8.561,8.535,57.483,4347826087067.848,100000000000135.81]
print(s)
choose(s)
print("因子PH值统计值")
print(avg_ph_list )
...
print(cou_ph_list )
choose(cou_ph_list)
s = [8.561,8.535,57.483,4347826087067.848,0,100000000000135.81]
choose(s)
print("--------------------------------------------")
