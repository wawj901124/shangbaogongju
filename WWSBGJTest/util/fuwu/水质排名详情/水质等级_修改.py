...
def connectDcloudBaseMyDBAndUpdate(sql):
    print("执行语句:%s" % sql)
    cursor_dcloud_base.execute(sql)
...
#根据支干/断面的ID修改td_station_state表获取对应STATION_ID为mn_id数据的 WATER_TYPE字段
# 定义要执行的SQL语句
xiugai_dengji = "6"
sql = """
UPDATE td_station_state SET WATER_TYPE = REPLACE(WATER_TYPE, "%s", "%s") WHERE STATION_ID='%s'
""" % (water_type,xiugai_dengji,mn_id)
connectDcloudBaseMyDBAndUpdate(sql)

water_type = connectDcloudBaseMyDBAndSelect(mysql_yuju)[0][5]
print(water_type)
print("修改后，%s（支干/断面）的水质等级为%s" % (zhigan_name,water_type))
