...
for i in range(5,shishiquxian_biaotou_len):
...
for i in range(0, shishiquxian_biaotou_list_len):
    if "RTD" in shishiquxian_biaotou_list[i]:
        ...
        biaotou_list_yinzi_name = connectDcloudBaseMyDBAndSelect(mysql_yuju)[0][0]
        # print(biaotou_list_yinzi_name)
        shishiquxian_biaotou_list[i] = "%s(%s)"%(shishiquxian_biaotou_list[i],biaotou_list_yinzi_name)
...
mysql_yuju = """
        SELECT * FROM td_raw_2011_%s WHERE STATION_ID='%s' ORDER by DT DESC limit 131
        """% (tbStationid,tbStationid)
# cursor_dcloud_base.execute(mysql_yuju)
# # 数据库提交命令
# # self.conn.commit()  # 执行update操作时需要写这个，否则就会更新不成功
# rs = cursor_dcloud_base.fetchall()  # 获取执行sql后的结果
...
for shishiquxian_one in shishiquxian:
    ...
    for i in range(5, shishiquxian_one_len):
